...
- Q2 Project angle exploration — conditional on `qna/<slug>/answer.md` having no draft yet (see Flow §1). Syllabus bucket is not asked here; it stays implicit in that question's `qc-eval.md` scope.
...
- `personal-context/` — `resume.md` · `stories/` (single source of truth for project facts).
...
## Guardrails (hard)
- →voice Hypothetical & synthetic scenarios are encouraged — that's how concepts get learned. No grounding requirement in discussion or in `answer.md` drafts.
